...
class TransactionServiceTest {
    ...
    @Test
    void createTransactionLog_TestValidCreation() {
        var sizeBefore = transactionRepository.findAll().size();
        Transaction testTransaction = transactionService.createTransactionLog(testAccount1, BigDecimal.valueOf(250));
        var sizeAfter = transactionRepository.findAll().size();
        assertEquals(sizeAfter, sizeBefore + 1);
    }

    @Test
    void createTransactionLog_TestValidName(){
        Transaction testTransaction = transactionService.createTransactionLog(testAccount1, BigDecimal.valueOf(250));
        assertTrue(testTransaction.getName().contains("US$ 250.00 credited on:"));
    }
}